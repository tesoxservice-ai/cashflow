      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-6 py-8">
        ...
        <div className="mb-8 flex items-start justify-between gap-4">
          ...
          <button
            ...
            className="shrink-0 inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2.5 rounded-lg transition-colors">
            {mostrarAlta ? (
              <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>Cancelar</>
            ) : (
              <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15"/></svg>Nueva obra</>
            )}
          </button>
        </div>
        ...
        {mostrarAlta && (
          <div className="bg-white border border-slate-200 rounded-xl p-5 mb-6">
            ...
            <form onSubmit={handleGuardarAlta} noValidate>
              ...
              {/* Fila 2: número de compra interno */}
              <div className="mb-4">
                <CampoTexto
        ...
        {cargando ? <EstadoCarga /> : obras.length === 0 ? <EstadoVacio /> : (
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            {errorEdicion && <div className="px-5 pt-4"><MensajeError mensaje={errorEdicion} onCerrar={() => setErrorEdicion('')} /></div>}
  ...
  return (
    <tr className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60 transition-colors">
      ...
      <td className="px-5 py-3.5 text-slate-500 whitespace-nowrap">
        {obra.numero_compra
          ? <span className="inline-block text-xs font-medium text-slate-600 bg-slate-100 px-2 py-0.5 rounded">{obra.numero_compra}</span>
      ...
      <td className="px-5 py-3.5">
        <div className="flex items-center justify-end gap-2 whitespace-nowrap">
          <button onClick={onEditar} disabled={editandoOtra || toggling}
            className="text-xs font-medium bg-blue-600 hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed text-white px-3 py-1.5 rounded-md transition-colors">
            Editar
          </button>
          {toggling ? (
            ...
            <button onClick={onToggle} disabled={editandoOtra}
              className={`text-xs font-medium text-white px-3 py-1.5 rounded-md transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${obra.activa ? 'bg-slate-400 hover:bg-slate-500' : 'bg-emerald-600 hover:bg-emerald-700'}`}>
...
function FilaEdicion({ obra, valores, guardando, onChange, onGuardar, onCancelar }) {
  const inputCls = `w-full px-2 py-1.5 text-sm rounded border border-blue-300 text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white`
  return (
    <tr className="border-b border-blue-200 bg-blue-50">
      ...
      <td className="px-3 py-2.5 min-w-[140px]">
        <input type="text" value={valores.numero_compra ?? ''} onChange={e => onChange('numero_compra', e.target.value)} className={inputCls} placeholder="Nº compra"/>
      </td>
      <td className="px-3 py-2.5 min-w-[140px]">
      ...
      <td className="px-3 py-2.5">
        <div className="flex items-center justify-end gap-2 whitespace-nowrap">
          ...
          <button onClick={onCancelar} disabled={guardando}
            className="text-xs font-medium bg-slate-300 hover:bg-slate-400 disabled:opacity-50 text-slate-700 px-3 py-1.5 rounded-md transition-colors">
            Cancelar
          </button>
        </div>
      </td>
...
function Th({ children, align = 'left' }) {
  return <th className={`px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide ${align === 'right' ? 'text-right' : 'text-left'}`}>{children}</th>
}
...
function MensajeError({ mensaje, onCerrar }) {
  return (
    <div className="flex items-start justify-between gap-3 bg-red-50 border border-red-100 text-red-700 text-sm rounded-lg px-4 py-3 mb-5">
